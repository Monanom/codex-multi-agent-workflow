...
一个可安装的 Codex Skill：根据任务复杂度协调主代理、可选的 Luna 执行代理与可选的 ChatGPT Pro 外部复核。
...
如果想把边界明确的执行任务交给 Luna，将 `skill/codex-multi-agent-workflow/templates/luna-worker.toml` 复制到 `~/.codex/agents/luna-worker.toml`。如果想让复杂任务得到外部复核，在 Codex 内置浏览器登录自己的 ChatGPT Pro，并在协作前确认 Intelligence 为 Extra High。
